...
.filter(([sk, s]) => Object.keys(s.setLogs).some(k => k.startsWith(exName + “**”)))
.sort((a, b) => {
...
1. National restaurant chains → use the chain’s PUBLISHED nutrition data. Include the brand in the name. Examples:
...
Return ONLY the JSON array, no markdown, no explanation.`, messages:[{role:"user",content:queryStr}] }) }); const d=await r.json(); const raw=d.content?.[0]?.text||"[]"; const s=raw.indexOf("["),e2=raw.lastIndexOf("]"); const items=s>=0&&e2>s?JSON.parse(raw.slice(s,e2+1)):[]; setResults(items.filter(x=>x.name&&x.cal>=0)); }catch(e){setResults([]);} setSearching(false); }; const pct=(v,t)=>t?Math.min(100,(v/t)*100):0; return( <div style={{height:"100%",overflowY:"auto",padding:14}}> <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}> <button onClick={()=>setDate(d=>{const n=new Date(d);n.setDate(n.getDate()-1);return n;})} style={hbtn}>PREV</button> <div style={{...IMP,fontSize:14,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>{lbl2}</div> <button onClick={()=>setDate(d=>{const n=new Date(d);n.setDate(n.getDate()+1);return n;})} style={hbtn}>NEXT</button> </div> <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7,marginBottom:11}}> {[[Math.round(tc),"Cal",C.acc,profile.targetCals],[Math.round(tp)+"g","Prot",C.a3,profile.protein],[Math.round(tcb)+"g","Carb",C.a6,profile.carbs],[Math.round(tf)+"g","Fat",C.a5,profile.fat]].map(([v,l,col,t])=>( <div key={l} style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:6,padding:“8px 6px”,textAlign:“center”}}>
...
Always tailor exercise and programming advice to the athlete’s training style.`; }; const send=async(m)=>{ if(!m.trim()||loading)return; const nc=[...conv,{role:"user",content:m}];setConv(nc);setMsg("");setLoading(true); try{const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:sys(),messages:nc.slice(-20)})});const d=await r.json();setConv(c=>[...c,{role:"assistant",content:d.content?.[0]?.text||"Error."}]);}catch{setConv(c=>[...c,{role:"assistant",content:"Connection error."}]);} setLoading(false); }; useEffect(()=>{if(ref.current)ref.current.scrollTop=ref.current.scrollHeight;},[conv,loading]); const qb=[ ["THIS WEEK","Explain this week's phase, what changed from last week, and key focus"], ["MY SESSIONS","Review my recent logged sessions. How are my lifts progressing? Any stalls?"], ["FILL MACROS","Based on what I've eaten today, what should I eat to hit my remaining macro targets?"], ["WEIGHT TREND","Analyze my recent weight entries. Am I on track for my goal?"], ["PROGRESS","Based on my logged weights, how should I progress my lifts this week?"], ["CARDIO","Cardio protocol for today based on my goal and current week"], ["RECOVERY","I'm sore — should I rest or train? What's best for my goal right now?"], ["MEALS","High protein meal ideas tailored to my goal and macros"] ]; return( <div style={{position:"fixed",bottom:0,right:0,width:"100%",maxWidth:420,height:"68vh",background:C.s1,borderTop:`2px solid ${C.acc}`,borderLeft:`1px solid ${C.bd}`,borderRadius:"12px 0 0 0",zIndex:300,display:"flex",flexDirection:"column",transform:open?"translateY(0)":"translateY(100%)",transition:"transform .3s ease",boxShadow:"-4px -4px 30px rgba(0,0,0,.6)"}}> <div style={{padding:"9px 13px",borderBottom:`1px solid ${C.bd}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}> <div style={{...IMP,fontSize:13,fontWeight:800,letterSpacing:2,textTransform:"uppercase",color:C.acc}}>RECOMP COACH</div> <button onClick={onClose} style={{background:"transparent",border:`1px solid ${C.bd}`,color:C.mu,width:26,height:26,borderRadius:4,fontSize:15,cursor:"pointer"}}>✕</button> </div> <div style={{padding:"7px 11px",borderBottom:`1px solid ${C.bd}`,display:"flex",gap:4,overflowX:"auto",flexShrink:0}}> {qb.map(([l,m])=><button key={l} onClick={()=>send(m)} style={{...hbtn,whiteSpace:"nowrap",fontSize:10,padding:"3px 8px"}}>{l}</button>)} </div> <div ref={ref} style={{flex:1,overflowY:"auto",padding:11}}> <div style={{display:"flex",flexDirection:"column",gap:9}}> {conv.map((m,i)=>( <div key={i} style={{display:"flex",gap:7,flexDirection:m.role==="user"?"row-reverse":"row"}}> <div style={{width:27,height:27,borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",...IMP,fontWeight:900,fontSize:10,flexShrink:0,background:m.role==="assistant"?`linear-gradient(135deg,${C.acc},${C.a2})`:C.s3,color:m.role==="assistant"?"#000":C.tx,border:m.role==="user"?`1px solid ${C.bd}`:"none"}}> {m.role==="assistant"?"AI":(profile.name?.[0]?.toUpperCase()||"U")} </div> <div style={{maxWidth:"84%"}}><div style={{padding:"8px 10px",borderRadius:5,fontSize:12,lineHeight:1.6,background:m.role==="assistant"?"#13131a":C.s3,border:`1px solid ${C.bd}`,borderLeft:m.role==="assistant"?`3px solid ${C.acc}`:`1px solid ${C.bd}`}} dangerouslySetInnerHTML={{__html:m.content.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\*\*(.*?)\*\*/g,"<strong style='color:#e8ff47'>$1</strong>").replace(/\n/g,"<br>")}}/></div> </div> ))} {loading&&<div style={{display:"flex",gap:7,alignItems:"center"}}> <div style={{width:27,height:27,borderRadius:4,background:`linear-gradient(135deg,${C.acc},${C.a2})`,display:"flex",alignItems:"center",justifyContent:"center",...IMP,fontWeight:900,fontSize:10,color:"#000"}}>AI</div> <div style={{padding:"8px 10px",borderRadius:5,background:"#13131a",border:`1px solid ${C.bd}`,borderLeft:`3px solid ${C.acc}`,display:"flex",gap:4,alignItems:"center"}}> {[0,.2,.4].map((d,i)=><div key={i} style={{width:4,height:4,background:C.mu,borderRadius:"50%",animation:`pulse ${1.2}s ${d}s ease-in-out infinite`}}/>)} </div> </div>} </div> </div> <div style={{background:C.bg,borderTop:`1px solid ${C.bd}`,padding:"9px 11px",display:"flex",gap:6,alignItems:"flex-end",flexShrink:0}}> <textarea value={msg} onChange={e=>setMsg(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send(msg);}}} placeholder="Ask your coach..." rows={1} style={{flex:1,background:C.s2,border:`1px solid ${C.bd}`,color:C.tx,padding:“7px 9px”,borderRadius:4,fontSize:12,resize:“none”,minHeight:34,maxHeight:85,outline:“none”,lineHeight:1.5,fontFamily:“inherit”}}/>
